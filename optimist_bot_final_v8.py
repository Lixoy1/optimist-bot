#!/usr/bin/env python3
"""
Оптимист v8 — ФИНАЛЬНАЯ ВЕРСИЯ
- Inline меню исчезает после выбора
- Интенсивность ответов (activity_level)
- Реакции по контексту (позитив/негатив/вопрос)
- Двойная LLM (Groq + Gemini)
- Все функции из исходного бота
"""
...
logger = logging.getLogger("OPTIMIST_v8")
...
# ==================== НАСТРОЙКИ ====================
SETTINGS_FILE = "bot_settings_v8.json"
...
MOODS = {
    ...
    "mafioso": {
        "name": "🔪 Мафиози",
        "emoji": "🕴️",
        "prompt": "Ты — авторитетный мафиози. Говоришь по понятиям, используешь жаргон: братва, развод, чисто."
    }
}
...
async def change_mood(call: CallbackQuery):
    ...
    save_settings()
    # Убираем клавиатуру, оставляем только текст
...
async def show_settings(call: CallbackQuery):
    ...
    activity_name = ACTIVITY_LEVELS.get(s["activity_level"], "Средне")
...
async def toggle_activity(call: CallbackQuery):
    cid = str(call.message.chat.id)
    s = chat_settings[cid]
    activities = list(ACTIVITY_LEVELS.keys())
    current = activities.index(s["activity_level"])
    s["activity_level"] = activities[(current + 1) % len(activities)]
    save_settings()
    await show_settings(call)
...
async def cmd_draw(message: types.Message):
    ...
    url = await generate_image_url(prompt, "реализм, детализировано")
